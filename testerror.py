...
class MNIST_FullyConnected(nn.Module):
    # A fully-connected neural network model designed for the MNIST dataset.
...
new_data = torchvision.datasets.MNIST('./new_data', train=False, download=True, transform=torchvision.transforms.ToTensor())
dl_new_data = torch.utils.data.DataLoader(new_data, batch_size=10000, shuffle=False)
...
with torch.no_grad():
    ...
    for features_, labels_ in dl_new_data:
        features, labels = torch.reshape(features_, (-1, 28 * 28)).to(DEVICE), labels_.to(DEVICE)
        pred = mw.forward(features)
        loss = F.nll_loss(pred, labels)
        running_loss += loss.item() * features_.size(0)
